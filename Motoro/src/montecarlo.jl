...
model = RiskNeutralMonteCarlo(100, 10_000, VarianceReduction(PseudoRandom(), Antithetic()))
price(call, model, data)

model = RiskNeutralMonteCarlo(100, 10_000, VarianceReduction(Stratified(), NoPairing()))
price(call, model, data)
```
